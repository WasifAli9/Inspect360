...
- **Team-Based Work Order Management**: Complete team management system for work order assignment and notifications. Features include team creation with distribution email lists, flexible member management (both staff users and contractor contacts), maintenance category assignment for automatic work order routing, comprehensive Settings UI with batched operations, role-based access control (admin/owner only), organization scoping, and server-side batched updates via /api/teams/:id/full endpoint. Known limitation: Operations are batched server-side but not wrapped in database transactions, so partial failures may occur in edge cases.
