...
- **PDF Generation**: Server-side PDF generation using Puppeteer and Chromium for professional inspection reports with full image loading and text formatting. **Security**: Implements comprehensive XSS protection with HTML escaping for all user content and strict URL whitelisting (allows only https://, http://, and safe data:image types - blocks SVG/javascript/vbscript). **Image Handling**: Relative photo URLs automatically converted to absolute URLs via request-derived base URL, ensuring all inspection photos load correctly in PDFs. **Text Formatting**: Preserves AI-generated note formatting with line breaks while maintaining security through HTML escaping then converting newlines to `<br>` tags. **Backend**: PDF service (`server/pdfService.ts`) with `escapeHtml()`, `sanitizeUrl()`, and `formatText()` utilities, endpoint `/api/inspections/:id/pdf` with organization isolation. **Frontend**: Print button downloads PDF with success/error toast notifications. **Output**: Branded cover page with Inspect360 logo, property information grid, section headers, field values with fully loaded photos, properly formatted ratings/notes, all professionally formatted for A4 printing (~69KB average file size).
